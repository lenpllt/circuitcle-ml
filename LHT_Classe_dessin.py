...
class Dessin:
    ...
    def __init__(self, canvas):
        ...
        self.canvas.bind_all("<KeyPress-+>", self.zoom_in)
        self.canvas.bind_all("<KeyPress-minus>", self.zoom_out)
   ...
   #dessine un fil horizontal de la longueur de la case et un fil vertical partant du milieu de la case et dirigé vers le bas :  pour les case départ vers une cellule et liant le jeu de barres
    ...
    #le même que le précédent mais retourné de 180°
    def dessiner_fil_h_arrivee(self, x, y, taille):
        self.canvas.create_line(x, y + taille/2, x + taille, y + taille/2, fill='black', width=2)
    ...
    def dessiner_fusible(self, x, y, taille):
        ...
        self.canvas.create_text(x-2*taille/3 , y + taille/2, text='Fusible', fill='black',font=("Arial", taille//5))
    
    #dessine une eclisse quand les morceaux de cuivre sont presents, en écrivant "eclisse" à gauche du dessin
    def dessiner_eclisse_presente(self, x, y, taille):
        self.canvas.create_line(x -2, y, x -2 , y + taille, fill='black', width=2)
        self.canvas.create_line(x +2, y, x +2 , y + taille, fill='black', width=2)
        # dessine les arcs de cercles aux extrémités
        self.canvas.create_arc(x - taille / 5 , y, x + taille / 5 , y + taille / 4, start=0, extent=180, style=tk.ARC, width=2)
        self.canvas.create_arc(x - taille / 5 , y + 3 * taille / 4, x + taille / 5 , y + taille, start=180, extent=180, style=tk.ARC, width=2)
        
        #ajoute un texte au centre de l'éclisse
        self.canvas.create_text(x - 2*taille/3, y + taille / 2, text='Éclisse', fill='black', font=("Arial", taille // 5))
        

    #dessine une eclisse quand les morceaux de cuivre sont absents, en écrivant "eclisse" à gauche du dessin
    def dessiner_eclisse_absente(self, x, y, taille):
        #dessine les arcs de cercles aux extrémités
        self.canvas.create_arc(x - taille / 5 , y, x + taille / 5 , y + taille / 4, start=0, extent=180, style=tk.ARC, width=2)
        self.canvas.create_arc(x - taille / 5 , y + 3 * taille / 4, x + taille / 5 , y + taille, start=180, extent=180, style=tk.ARC, width=2)
        self.canvas.create_text(x - 2*taille/3, y + taille / 2, text='Éclisse', fill='black', font=("Arial", taille // 5))
        
    #ecris le texte "mot" centré en (x,y)
    def ecrire_mot(self, x, y, taille, mot):
        self.canvas.create_text( x, y, text=mot, fill='black',font=("Arial", taille//5))

        
    #dessine une terre, en écrivant "terre" en dessous du dessin
    def dessiner_terre(self, x, y, taille):
        self.canvas.create_line(x, y , x + 6*taille/9, y, fill='black', width=2)
        self.canvas.create_line(x + 6*taille/9, y-taille/3, x + 6*taille/9, y + taille/3, fill='black', width=2)
        self.canvas.create_line(x + 7*taille/9, y - taille/4, x + 7*taille/9 , y + taille/4, fill='black', width=2)
        self.canvas.create_line(x + 8*taille/9, y - taille/6, x + 8*taille/9 , y + taille/6, fill='black', width=2)
        self.canvas.create_text(x + 4*taille/5 , y + taille/2, text='Terre', fill='black',font=("Arial", taille//5))
    ...
    #dessine une serrure mere en distinguant une armoire (plastron pour le CC) et une serrure normale
    def dessiner_serrure_mere(self, x, y, taille, taille_serrure_mere, cellule):
        nombre_serrures = len(cellule.serrures)
        largeur = len(cellule.matrice[0])
        position = 0
        for serrure in cellule.serrures: #positionne les différentes serrures mères côte à côte 
            for i in range(len(serrure.clefs)): #positionne les différentes clefs d'un même serrure mère les unes en dessous des autres
                self.dessiner_carre(
                               x +(position+1)*taille*largeur/(nombre_serrures+1),
                ...
                self.canvas.create_text(
                                   ...
                                   font=("Arial", taille_serrure_mere//16)
                                   )
            self.dessiner_rectangle( #dessine un rectangle entourant la serrure mère
                x +(position+1)*taille*largeur/(nombre_serrures+1)-13,
                y -taille_serrure_mere*9.4/10,
                x +(position+1)*taille*largeur/(nombre_serrures+1)+13,
                y -taille_serrure_mere*1/6
                )
            if serrure.type == 'serrure_mere':
                self.canvas.create_text(
                    x +(position+1)*taille*largeur/(nombre_serrures+1),
                    y -taille_serrure_mere*1/10,
                    text=serrure.nom,
                    fill='black',
                    font=("Arial", taille//5)
                )
            ...
            #si c'est une armoire, la serrure mère oriente le contrôle commande sur LHC ou LHT : on écrit alors l'orientation du CC
            if serrure.type == 'armoire':
                self.canvas.create_text(
                    x + taille*largeur/2,
                    y-taille_serrure_mere/10,
                    text="Mode : " +serrure.mode_CC,
                    font=("Arial", taille_serrure_mere//12)
                    )    
        # Rectangle entourant les serrures mères
        self.dessiner_rectangle(
            x + taille/5,
            y - taille_serrure_mere,
            x + taille*largeur - taille/5,
            y -taille/10
            )
    ...
    #dessine une source : c'est simplement un cercle avec marqué le nom de la source à l'intérieur
    def dessiner_source(self, source, x, y, taille):
        self.dessiner_cercle(x + taille/2 , y + taille/2, taille / 2)
        self.canvas.create_text(x + taille/2, y + taille/2, text=source.nom, fill='black', font=("Arial", taille // 5))
    ...
    #créé la couleur du carré d'une clef en fonction de son état 
    ...
    def dessiner_penne_disj(self,x,y, clef, taille):
        if clef.penne == 'rentre': #si il est rentre, c'est un petit rectangle
            self.canvas.create_rectangle(x-taille/5+taille/12, y-taille/5-taille/25, x+taille/5-taille/12, y-taille/5, fill='black')
        if clef.penne == 'sorti': #si il est sorti, le rectangle est plus grand
    ...
    #dessine le carré entourant le nom de la clef : ici c'est un rectangle car les noms de clefs sont longs
    def dessiner_carre(self,x, y, clef, taille):
        self.canvas.create_rectangle(x-taille/5-3, y-taille/5, x+taille/5+3, y+taille/5, fill=self.couleur_carre(clef))
    ...
    #dessine la boite a eclisse pour une voie (A ou B)
    def dessiner_boite(self, boite_eclisse, x, y, taille):
        if boite_eclisse.stock_eclisses > 0: #si la boite contient des eclisses, alors on les dessine
            self.canvas.create_line(x -taille/2 +5 , y - taille, x -taille/2 +5 , y + taille, fill='black', width=1)
            self.canvas.create_line(x -taille/2 +8 , y - taille, x -taille/2 +8 , y + taille, fill='black', width=1)
        self.canvas.create_line(x -taille/2 , y - taille , x , y - taille, fill='black', width=2)
        self.canvas.create_line(x - taille/2, y + taille , x , y + taille, fill='black', width=2)
        self.canvas.create_line(x - taille/2 , y - taille  , x -taille/2, y + taille, fill='black', width=2)
        self.canvas.create_text( x + taille/2, y + taille, text=boite_eclisse.nom, font=("Arial", taille//5))
        ...
        for ligne in range(len(matrice)):
            for colonne in range(len(matrice[0])):
                ...
                if isinstance(matrice[ligne][colonne], str):
                    ...
                    self.dessiner_carre(
                                   x + (colonne-y_smalt)*taille/2+taille/4 + a +3,
                                   y + (ligne-x_smalt)*taille/2-taille/4 + b ,
                                   smalt.chercher_clef_par_nom(matrice[ligne][colonne]),
                                   taille
                                   )
                    self.canvas.create_text(x + (colonne-y_smalt)*taille/2+taille/4 + a + 3,
                                       y + (ligne-x_smalt)*taille/2-taille/4 + b ,
                                       text=matrice[ligne][colonne], 
                                       fill='black',
                                       font=("Arial", taille//5)
                                       )
    #zoom le dessin de 10%
    def zoom_in(self, event=None):
        self.apply_zoom(1.1)  # Zoom avant de 10%

    #dezoome le dessin de 10%
    def zoom_out(self, event=None):
        self.apply_zoom(0.9)  # Zoom arrière de 10%
    ...
    def apply_zoom(self, scale):
        ...
        self.update_text_size(scale)

    #permet de zoomer le texte de la même manière que le dessin
    def update_text_size(self, scale):
        for item_id in self.canvas.find_all():
            if self.canvas.type(item_id) == "text": #regarde si l'élément dans la feuille de dessin est du texte 
                current_font = self.canvas.itemcget(item_id, "font") #récupère la police du texte
                ...
                if isinstance(current_font, str): 
                    ...
                    if len(font_parts) > 1 :
                        ...
                        if item_id not in self.text_sizes:
                            self.text_sizes[item_id] = font_size

                        real_font_size = self.text_sizes[item_id]
                        
                        #on calcule la nouvelle taille avec la valeur de scale
                        new_font_size = real_font_size * scale
                        new_font_size = max(new_font_size, 2)  # Taille minimale
                        self.text_sizes[item_id] = new_font_size
                        
                        #on applique la nouvelle taille (arrondie) à l'affichage
                        new_font = f"{font_family} {round(new_font_size)}"
