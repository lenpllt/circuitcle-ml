...
class ModifsExcel:
    ...
    def __init__(self, window):
        ...
        try:
            with open("dataLHC/dernier_palier_choisi_LHC.txt", "r") as file:
                self.palier = file.read().strip()
        except FileNotFoundError:
            self.palier = "1400_postVD"  #si le fichier n'est pas trouvé, on met par défaut le palier 1400
        print(self.palier)
    ...
    def create_palier_tab(self):
        ...
        tk.Label(self.palier_tab, text="Choisissez un palier (900 ou 1300 ou 1400) et l'état des modifications:").pack(padx=10, pady=10)
        ...
        try:
            ...
            dernier_palier_choisi = "1400_postVD"  #valeur par défaut si le fichier n'existe pas
        
        self.palier_var = tk.StringVar(value=dernier_palier_choisi)
        palier_menu = ttk.OptionMenu(self.palier_tab, self.palier_var, dernier_palier_choisi, "900_preVD", "900_postVD", "1300_preVD", "1300_postVD", "1400_preVD", "1400_postVD")
        palier_menu.pack(padx=10, pady=10)
        
        submit_button = ttk.Button(self.palier_tab, text="Soumettre", command=self.set_palier)
        submit_button.pack(padx=10, pady=10)

        tableau_button = ttk.Button(self.palier_tab, text="Visualisation tableau", command=self.tableauLHC_tab)
    ...
    def set_palier(self):
        ...
        with open("dataLHC/dernier_palier_choisi_LHC.txt", "w") as file:
    ...
    def create_key_tab(self):
        ...
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        ...
        for i, label in enumerate(self.key_labels):
            if label == "penne":
                ...
                etat_menu = ttk.OptionMenu(self.key_tab, self.key_type_var, "serrure_mere", "serrure_mere", "partie_mobile", "smalt", "coffret", "transformateur", "panneau")
                etat_menu.grid(row=i, column=1, padx=10, pady=5)
        ...
        self.reset_key_button = ttk.Button(self.key_tab, text="Réinitialiser", command=self.reset_key)
        self.reset_key_button.grid(row=len(self.key_labels) + 2, columnspan=2, padx=10, pady=10)
    ...
    #Recharge les données du fichier excel
    def refresh_key_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "clef"
        ...
        if sheet_name in book.sheetnames:
            ...
            noms = list(df['nom'].dropna().unique())
            appartenance = list(df['appartenance'].dropna().unique())
            cellules = list(df['cellule'].dropna().unique())

            self.coffret_entries["nom"]['values'] = noms
            self.key_entries["appartenance"]['values'] = appartenance
            self.coffret_entries["cellule"]['values'] = cellules
    ...
    def choisir_ligne(self, rows):
        ...
        def on_select():
            ...
            if ligne:
                ...
                print(f"Ligne sélectionnée : {self.selected_row_index}")
            else:
                dialog.selected_index = None
            dialog.destroy()
    ...
    def save_keys(self):
        ...
        if self.selected_row_index is not None:
            ...
            print(new_key)
            print(f"Clé modifiée dans {self.palier}_modifs_LHC.xlsx, feuille 'clef'")
            self.selected_row_index = None
        else:
            #Ajoute une nouvelle ligne
            df = df.append(new_key, ignore_index=True)
            print(f"Nouvelles clés sauvegardées dans {self.palier}_modifs_LHC.xlsx, feuille 'clef'")
        ...
        if new_key["type"] in ["partie_mobile", "smalt"]:
    ...
    def delete_key(self):
        ...
        for _, row in rows_to_delete.iterrows():
            ...
            if type_element in ["partie_mobile", "smalt"]:
        ...
        self.key_type_var.set("serrure_mere")

        for label in ["nom", "appartenance", "cellule"]:
            self.key_entries[label].set("")
    ...
    def update_representation(self, book, type_element, appartenance):
        ...
        #parcourt toutes les clés dans la feuille Excel et les places à la bonne position selon leurs états
        ...
        book.remove(sheet)
        with pd.ExcelWriter(f"dataLHC/{self.palier}_modifs_LHC.xlsx", engine='openpyxl') as writer:
            writer.book = book
            df.to_excel(writer, sheet_name=sheet_name, index=False)
            print(df)
    ...
    #réinitialise les champs de l'onglet clé
    def reset_key(self) : 
        print(f"Réinitialisation des champs. selected_row_index = {self.selected_row_index}")
        self.key_etat_var.set("prisonniere")
        self.penne_var.set("rentre")
        self.key_type_var.set("serrure_mere")
        ...
        self.selected_row_index = None
        print(f"Champs réinitialisés et selected_row_index remis à None. selected_row_index = {self.selected_row_index}")
    ...
    def create_coffret_tab(self):
        ...
        self.reset_coffret_button = ttk.Button(self.coffret_tab, text="Réinitialiser", command=self.reset_coffret)
        self.reset_coffret_button.grid(row=len(self.coffret_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    #Recharge les données du fichier excel
    def refresh_coffret_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "coffret"
    ...
    def delete_coffret(self):
        ...
        if row_index:
            ...
            for label in ["nom", "cellule"]:
                self.coffret_entries[label].set("")

            self.refresh_coffret_comboboxes()
    ...
    def save_coffret(self):
        ...
        if self.coffret_row_index:
            ...
            print(f"Modification coffret dans {self.palier}_modifs_LHC.xlsx, feuille 'coffret'")
            ...
            if clef_sheet_name in book.sheetnames:
                ...
                with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
                    ...
                    clef_df.to_excel(writer, sheet_name=clef_sheet_name, index=False)
                    writer.save()
                print(f"Les clefs dans {self.palier}_modifs_LHC.xlsx, feuille 'clef' ont été mises à jour avec le nouveau nom du coffret")
            
            self.coffret_row_index = []
        else:
            df = df.append(new_coffret, ignore_index=True)
            print(f"Nouveaux coffrets sauvegardés dans {self.palier}_modifs_LHC.xlsx, feuille 'coffret'")
    ...
    #réinitialise les champs de l'onglet Coffret
    def reset_coffret(self) : 
        print(f"Réinitialisation des champs. coffret_row_index = {self.coffret_row_index}")
        self.type_coffret_var.set("Coffret")

        for label in ["nom", "cellule"]:
            self.coffret_entries[label].set("")
        
        self.coffret_row_index = None
        print(f"Champs réinitialisés et coffret_row_index remis à None. offret_row_index = {self.coffret_row_index}")

    #crée l'onglet permettant de modifier les fusibles
    def create_fusible_tab(self):
        self.fusible_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.fusible_tab, text="Fusible")
        ...
        for i, label in enumerate(self.fusible_labels):
            ...
            if label == "type":
                ...
                # Combobox pour "nom"
                self.fusible_nom_var = tk.StringVar()
                nom_combobox = ttk.Combobox(self.fusible_tab, textvariable=self.fusible_nom_var, values=noms_existants)
                nom_combobox.grid(row=i, column=1, padx=10, pady=5)
                self.fusible_entries[label] = nom_combobox
                nom_combobox.bind("<<ComboboxSelected>>", self.check_fusible_existence)
            elif label == "cellule":
                # Combobox pour "cellule"
                self.fusible_cellule_var = tk.StringVar()
                cellule_combobox = ttk.Combobox(self.fusible_tab, textvariable=self.fusible_cellule_var, values=cellules_existantes)
                cellule_combobox.grid(row=i, column=1, padx=10, pady=5)
                self.fusible_entries[label] = cellule_combobox

        self.save_fusible_button = ttk.Button(self.fusible_tab, text="Sauvegarder", command=self.save_fusible)
        ...
        self.reset_fusible_button = ttk.Button(self.fusible_tab, text="Réinitialiser", command=self.reset_fusible)
        self.reset_fusible_button.grid(row=len(self.fusible_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    def check_fusible_existence(self, event=None):
        ...
        self.fusible_row_index = df.index[df['nom'] == nom_to_load].tolist()
        
        if self.fusible_row_index:
            # Charge le fusible existant
            row = df.iloc[self.fusible_row_index[0] - 1]
            self.fusible_entries["cellule"].set(row["cellule"])
            self.delete_fusible_button.config(state=tk.NORMAL)

    #Recharge les données du fichier excel
    def refresh_fusible_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "fusible"
        ...
        if sheet_name in book.sheetnames:
            ...
            self.fusible_entries["nom"]['values'] = noms
            self.fusible_entries["cellule"]['values'] = cellules
    ...
    def delete_fusible(self):
        ...
        if row_index:
            ...
            if clef_sheet_name in book.sheetnames:
                ...
                #Supprime les clefs qui ont comme appartenance le nom du fusible
                clef_df = clef_df[clef_df['appartenance'] != nom]
            ...
            print(f"Fusible et ses clefs supprimés dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")

            for label in ["nom", "cellule"]:
                self.fusible_entries[label].set("")

            self.refresh_fusible_comboboxes()

    #sauvegarde les modifications 
    #si un fusible existe avec ce nom, il le modifie, sinon il crée un nouveau fusible      
    def save_fusible(self):
        ...
        if self.fusible_row_index:
            old_nom = df.loc[self.fusible_row_index[0], 'nom']
            #Modifie le fusible existant
            df.loc[self.fusible_row_index[0]] = new_fusible
            print(f"Modification fusible dans {self.palier}_modifs_LHC.xlsx, feuille 'fusible'")
            ...
            if clef_sheet_name in book.sheetnames:
                ...
                clef_df.loc[clef_df['appartenance'] == old_nom, 'appartenance'] = new_fusible['nom']
                ...
                print(f"Les clefs dans {self.palier}_modifs_LHC.xlsx, feuille 'clef' ont été mises à jour avec le nouveau nom du fusible")
            else:
                messagebox.showwarning("Avertissement", "Feuille 'clef' non trouvée")
            self.fusible_row_index = []
        else:
            df = df.append(new_fusible, ignore_index=True)
            print(f"Nouveaux fusiblex sauvegardés dans {self.palier}_modifs_LHC.xlsx, feuille 'fusible'")
    ...
    #réinitialise les champs de l'onglet Fusible
    def reset_fusible(self) : 
        print(f"Réinitialisation des champs. fusible_row_index = {self.fusible_row_index}")

        for label in ["nom", "cellule"]:
            self.fusible_entries[label].set("")
        
        self.fusible_row_index = None
        print(f"Champs réinitialisés et fusible_row_index remis à None. fusible_row_index = {self.fusible_row_index}")

    # Crée l'onglet permettant de modifier un panneau
    def create_panneau_tab(self):
        self.panneau_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.panneau_tab, text="Panneaux")
        ...
        self.save_panneau_button = ttk.Button(self.panneau_tab, text="Sauvegarder", command=self.save_panneaux)
        ...
        self.reset_panneau_button = ttk.Button(self.panneau_tab, text="Réinitialiser", command=self.reset_panneau)
        self.reset_panneau_button.grid(row=len(self.panneau_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    #Recharge les données du fichier excel
    def refresh_panneau_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "panneau"
        ...
        if sheet_name in book.sheetnames:
            ...
            self.panneau_entries["nom"]['values'] = noms
            self.panneau_entries["cellule"]['values'] = cellules
    ...
    def delete_panneau(self):
        ...
        if row_index:
            ...
            print(f"Cellule supprimée dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
            
            #Réinitialise les champs de saisie
            for label in ["nom", "cellule"]:
                self.panneau_entries[label].set("")

            self.refresh_panneau_comboboxes()

    #sauvegarde les modifications 
    #si un panneau existe avec ce nom, il le modifie, sinon il crée un nouveau panneau 
    def save_panneaux(self):
        ...
        if self.panneau_row_index:
            ...
            print(f"Modification panneau dans {self.palier}_modifs_LHC.xlsx, feuille 'panneau'")
            ...
            if clef_sheet_name in book.sheetnames:
                ...
                print(f"Les clefs dans {self.palier}_modifs_LHC.xlsx, feuille 'clef' ont été mises à jour avec le nouveau nom du panneau")
            else:
                messagebox.showwarning("Avertissement", "Feuille 'clef' non trouvée")
            self.panneau_row_index = []
        else:
            df = df.append(new_panneau, ignore_index=True)
            print(f"Nouveaux panneaux sauvegardés dans {self.palier}_modifs_LHC.xlsx, feuille 'panneau'")
    ...
    #réinitialise les champs de l'onglet panneau
    def reset_panneau(self) : 
        print(f"Réinitialisation des champs. panneau_row_index = {self.panneau_row_index}")
        self.panneau_etat_var.set("ferme")

        for label in ["nom", "cellule"]:
            self.panneau_entries[label].set("")
        
        self.panneau_row_index = None
        print(f"Champs réinitialisés et panneau_row_index remis à None. panneau_row_index = {self.panneau_row_index}")
    ...
    def create_transformateur_tab(self):
        ...
        self.reset_transformateur_button = ttk.Button(self.transformateur_tab, text="Réinitialiser", command=self.reset_transformateur)
        self.reset_transformateur_button.grid(row=len(self.transformateur_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    #Recharge les données du fichier excel
    def refresh_transformateur_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "transformateur"
        ...
        if sheet_name in book.sheetnames:
            ...
            self.transformateur_entries["nom"]['values'] = noms
            self.transformateur_entries["cellule"]['values'] = cellules
    ...
    def delete_transformateur(self):
        ...
        if row_index:
            ...
            print(f"Transformateur supprimé dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
            
        for label in ["nom", "cellule"]:
            self.transformateur_entries[label].set("")

        self.refresh_transformateur_comboboxes()
    ...
    def save_transformateur(self):
        ...
        if self.transformateur_row_index:
            ...
            print(f"Modification transformateur dans {self.palier}_modifs_LHC.xlsx, feuille 'transformateur'")
        ...
        print(f"Nouveaux transformateurs sauvegardés dans {self.palier}_modifs_LHC.xlsx, feuille 'transformateur'")

        self.refresh_transformateur_comboboxes()

    #réinitialise les champs de l'onglet transformateur
    def reset_transformateur(self) : 
        print(f"Réinitialisation des champs. transformateur_row_index = {self.transformateur_row_index}")

        for label in ["nom", "cellule"]:
            self.transformateur_entries[label].set("")
        
        self.transformateur_row_index = None
        print(f"Champs réinitialisés et transformateur_row_index remis à None. transformateur_row_index = {self.transformateur_row_index}")
    ...
    def create_serrure_tab(self):
        ...
        self.reset_serrure_button = ttk.Button(self.serrure_tab, text="Réinitialiser", command=self.reset_serrure)
        self.reset_serrure_button.grid(row=len(self.serrure_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    #Recharge les données du fichier excel
    def refresh_serrure_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "serrure"
        ...
        if sheet_name in book.sheetnames:
            ...
            self.serrure_entries["nom"]['values'] = noms
            self.serrure_entries["cellule"]['values'] = cellules
    ...
    def delete_serrure(self):
        ...
        if row_index:
            ...
            print(f"Serrure supprimée dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
            
            #Réinitialise les champs de saisie
            for label in ["nom", "cellule"]:
                self.serrure_entries[label].set("")

        self.refresh_serrure_comboboxes()
    ...
    def save_serrures(self):
        ...
        if self.serrure_row_index:
            ...
            print(f"Modification serrure dans {self.palier}_modifs_LHC.xlsx, feuille 'serrure'")
            ...
            if clef_sheet_name in book.sheetnames:
                ...
                print(f"Les clefs dans {self.palier}_modifs_LHC.xlsx, feuille 'clef' ont été mises à jour avec le nouveau nom de la serrure")
            else:
                messagebox.showwarning("Avertissement", "Feuille 'clef' non trouvée")
            self.serrure_row_index = []
        else:
            df = df.append(new_serrure, ignore_index=True)
            print(f"Nouvelles serrures sauvegardées dans {self.palier}_modifs_LHC.xlsx, feuille 'serrure'")
    ...
    #réinitialise les champs de l'onglet serrure
    def reset_serrure(self) : 
        print(f"Réinitialisation des champs. serrure_row_index = {self.serrure_row_index}")
        self.type_serrure_var.set("serrure")

        for label in ["nom", "cellule"]:
            self.serrure_entries[label].set("")
        
        self.serrure_row_index = None
        print(f"Champs réinitialisés et serrure_row_index remis à None. serrure_row_index = {self.serrure_row_index}")

    '''
    #crée l'onglet permettant de modifier les sources
    def create_source_tab(self):
        self.source_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.source_tab, text="sources")
        
        self.source_labels = ["nom", "type", "cellule"]
        self.source_entries = {}
        
        # Liste des valeurs possibles pour 'nom' et 'cellule'
        excel_path = f"Data/{self.palier}_modifs_LHC.xlsx"
        sheet_name = "source"
        ...
        if sheet_name in book.sheetnames:
            ...
            df.columns = df.iloc[0]
            df = df[1:]
            possible_noms = df['nom'].tolist()
            possible_cellules = df['cellule'].tolist()
        else:
            messagebox.showwarning("Avertissement", "Fichier Excel non trouvé")
            return
        
        # Crée les labels et les comboboxes
        for i, label in enumerate(self.source_labels):
            ttk.Label(self.source_tab, text=label).grid(row=i, column=0, padx=10, pady=5)
            
            if label == "type":
                self.source_type_var = tk.StringVar(value="source")
                ttk.Label(self.source_tab, textvariable=self.source_type_var).grid(row=i, column=1, padx=10, pady=5)
            
            elif label == "nom":
                # Combobox pour 'nom', rempli avec les valeurs existantes
                combobox_nom = ttk.Combobox(self.source_tab, values=possible_noms)
                combobox_nom.grid(row=i, column=1, padx=10, pady=5)
                self.source_entries[label] = combobox_nom
                combobox_nom.bind("<FocusOut>", self.check_source_existence)
            
            elif label == "cellule":
                # Combobox pour 'cellule', rempli avec les valeurs existantes
                combobox_cellule = ttk.Combobox(self.source_tab, values=possible_cellules)
                combobox_cellule.grid(row=i, column=1, padx=10, pady=5)
                self.source_entries[label] = combobox_cellule

        self.save_source_button = ttk.Button(self.source_tab, text="Sauvegarder", command=self.save_sources)
        ...
        # Liste les noms des sources actuellement dans l'excel
        self.source_listbox_label = ttk.Label(self.source_tab, text="nom des sources existantes:")
        self.source_listbox_label.grid(row=len(self.source_labels) + 1, column=0, padx=10, pady=5)
        
        self.source_listbox = tk.Listbox(self.source_tab)
        self.source_listbox.grid(row=len(self.source_labels) + 2, columnspan=2, padx=10, pady=5)
        
        for nom in possible_noms:
            self.source_listbox.insert(tk.END, nom)

    #vérifie si une source existe avec ce nom, et la charge le cas échéant    
    def check_source_existence(self, event=None):
        ...
        if row_index:
            # Charge la source existante
            row = df.iloc[row_index[0] - 1]
            self.source_entries["cellule"].set(row["cellule"])  # Met à jour la combobox "cellule"
    ...
    def delete_source(self):
        ...
        if row_index:
            ...
            print(f"Source supprimée dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
    ...
    def save_sources(self):
        ...
        print(f"Nouvelles sources sauvegardées dans {self.palier}_modifs_LHC.xlsx, feuille 'source'")
    '''
    ...
    def delete_clefs_libres(self):
        ...
        if row_index:
            ...
            print(f"Clef supprimée dans {self.palier}_modifs_LHC.xlsx, feuille 'clefs_libres'")
    ...
    def save_clefs_libres(self):
        ...
        if new_clef_libre["clefs"] not in df["clefs"].values:
            ...
            print(f"Nouvelles clefs_libres sauvegardées dans {self.palier}_modifs_LHC.xlsx, feuille 'clefs_libres'")
        else:
    ...
    def create_partie_mobile_tab(self):
        ...
        for i, label in enumerate(self.partie_mobile_labels):
            ...
            if label == "type":
                ...
                self.partie_mobile_deplacement_var = tk.StringVar(value="droite")
                deplacement_menu = ttk.OptionMenu(self.partie_mobile_tab, self.partie_mobile_deplacement_var, "droite", "droite", "gauche")
        ...
        self.delete_partie_mobile_button = ttk.Button(self.partie_mobile_tab, text="Supprimer", command=self.delete_partie_mobile)
        self.delete_partie_mobile_button.grid(row=len(self.partie_mobile_labels), columnspan=3, padx=10, pady=10)

        self.reset_partie_mobile_button = ttk.Button(self.partie_mobile_tab, text="Réinitialiser", command=self.reset_partie_mobile)
        self.reset_partie_mobile_button.grid(row=len(self.partie_mobile_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    #Recharge les données du fichier excel
    def refresh_smalt_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "partie_mobile"
        ...
        if sheet_name in book.sheetnames:
            ...
            self.partie_mobile_entries["nom"]['values'] = noms
            self.partie_mobile_entries["cellule"]['values'] = cellules
    ...
    def save_partie_mobile(self):
        ...
        if self.partie_mobile_row_index:
            ...
            print(f"Modification partie_mobile dans {self.palier}_modifs_LHC.xlsx, feuille 'partie_mobile'")
            #Met à jour les clefs dans la feuille clef
            clef_sheet_name = "clef"
            if clef_sheet_name in book.sheetnames:
                clef_sheet = book[clef_sheet_name]
                ...
                print(f"Les clefs dans {self.palier}_modifs_LHC.xlsx, feuille 'clef' ont été mises à jour avec le nouveau nom du partie_mobile")
            else:
                messagebox.showwarning("Avertissement", "Feuille 'clef' non trouvée")
            self.partie_mobile_row_index = []
        else:
            df = df.append(new_partie_mobile, ignore_index=True)
            print(f"Nouveaux partie_mobilex sauvegardés dans {self.palier}_modifs_LHC.xlsx, feuille 'partie_mobile'")
        ...
        print(f"Nouvelles parties mobiles sauvegardées dans {self.palier}_modifs_LHC.xlsx, feuille 'partie_mobile'")

        self.refresh_smalt_comboboxes()
    ...
    def delete_partie_mobile(self):
        ...
        if row_index:
            ...
            print(f"Partie mobile supprimée dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
            
        self.partie_mobile_type_var.set("contacteur")
        self.partie_mobile_etat_var.set("ouvert")
        self.partie_mobile_position_var.set("embroche")
        self.partie_mobile_deplacement_var.set("droite")

        for label in self.partie_mobile_labels:
            if label in ["nom", "cellule"]:
                self.partie_mobile_entries[label].set("")
            elif label == "representation":
                self.partie_mobile_entries[label].delete(0, tk.END)
            elif label not in ["type", "etat", "position", "deplacement"]:
                self.partie_mobile_entries[label].delete(0, tk.END)
    ...
    def update_representation_partie_mobile(self, event=None):
        ...
        partie_mobile_nom = self.partie_mobile_entries.get("nom", "").get()
    ...
    #réinitialise les champs de l'onglet partie mobile
    def reset_partie_mobile(self):
        print(f"Réinitialisation des champs Partie Mobile. partie_mobile_row_index = {self.partie_mobile_row_index}")
        ...
        # Réinitialiser l'index de ligne sélectionné si existant
        self.partie_mobile_row_index = None
        print("Champs Partie Mobile réinitialisés et partie_mobile_row_index remis à None.")
    ...
    def create_smalt_tab(self):
        ...
        self.delete_smalt_button = ttk.Button(self.smalt_tab, text="Supprimer", command=self.delete_smalt)
        self.delete_smalt_button.grid(row=len(self.smalt_labels), columnspan=3, padx=10, pady=10)

        self.reset_smalt_button = ttk.Button(self.smalt_tab, text="Réinitialiser", command=self.reset_smalt)
        self.reset_smalt_button.grid(row=len(self.smalt_labels) + 1, columnspan=2, padx=10, pady=10)
    ...
    def check_smalt_existence(self, event=None):
        ...
        if self.smalt_row_index and 0 <= self.smalt_row_index[0] < len(df):
            ...
            self.smalt_entries["representation"].insert(0, str(row["representation"]))
    ...
    def refresh_smalt_comboboxes(self):
        ...
        if sheet_name in book.sheetnames:
            ...
            self.smalt_entries["nom"]['values'] = noms
            self.smalt_entries["cellule"]['values'] = cellules
    ...
    def save_smalt(self):
        ...
        if self.smalt_row_index:
            ...
            print(f"Modification smalt dans {self.palier}_modifs_LHC.xlsx, feuille 'smalt'")
            # Met à jour les clefs dans la feuille clef
            clef_sheet_name = "clef"
            if clef_sheet_name in book.sheetnames:
                clef_sheet = book[clef_sheet_name]
                ...
                print(f"Les clefs dans {self.palier}_modifs_LHC.xlsx, feuille 'clef' ont été mises à jour avec le nouveau nom du smalt")
            else:
                messagebox.showwarning("Avertissement", "Feuille 'clef' non trouvée")
            self.smalt_row_index = []
        else:
            df = df.append(new_smalt, ignore_index=True)
            print(f"Nouveaux smalts sauvegardés dans {self.palier}_modifs_LHT.xlsx, feuille 'smalt'")
        ...
        for _, row in df.iterrows():
            ...
            if appartenance == smalt_nom:
                ...
                if clef_nom:
                    ...
                    #met à jour la case "cellule" avec la valeur de la clé
                    self.smalt_entries["cellule"].delete(0, tk.END)
                    self.smalt_entries["cellule"].insert(0, cellule)
                    
        #met à jour le champ de saisie de la représentation
        self.smalt_entries["representation"].delete(0, tk.END)
    ...
    def delete_smalt(self):
        ...
        if row_index:
            ...
            print(f"Le smalt et les clés associées ont été supprimées dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
            
        self.smalt_type_var.set("smalt")
        self.smalt_etat_var.set("ouvert")
        self.smalt_deplacement_var.set("haut")

        for label in self.smalt_labels:
            if label in ["nom", "cellule"]:
                self.smalt_entries[label].set("")
            elif label == "representation":
    ...
    #réinitialise les champs de l'onglet smalt
    def reset_smalt(self):
        print(f"Réinitialisation des champs Partie Mobile. smalt_row_index = {self.smalt_row_index}")
        ...
        self.smalt_row_index = None
        print(f"Champs réinitialisés et smalt_row_index remis à None. {self.smalt_row_index}")
    ...
    def create_cellule_tab(self):
        ...
        # Mise à jour de la liste des champs, sans 'statut'
        self.cellule_labels = ["nom", "voisine", "position_x", "representation"]
        ...
        if sheet_name in book.sheetnames:
            ...
            noms_existants = df['nom'].unique().tolist()
            voisines_existantes = df['voisine'].unique().tolist()
        else:
            noms_existants = []
            voisines_existantes = []
        ...
        for i, label in enumerate(self.cellule_labels):
            ...
            if label == "nom":
                ...
                ttk.Label(self.cellule_tab, text="(Si la cellule est reliée avec une autre cellule autrement que par le jeu de barre, écrire le nom de cette autre cellule)").grid(row=i, column=2, padx=10, pady=5)
        ...
        self.reset_cellule_button = ttk.Button(self.cellule_tab, text="Réinitialiser", command=self.reset_cellule)
        self.reset_cellule_button.grid(row=len(self.cellule_labels) + 4, columnspan=3, padx=10, pady=10)
        ...
        self.canvas_matrix = tk.Canvas(self.canvas_frame, width=400, height=250)
    ...
    def load_and_draw_all_cells(self):
        ...
        #on parcourt toutes les cellules et les dessine
        for _, row in df.iterrows():
            nom = row["nom"]
            ...
            if 'LHC' in nom : 
                ...
                self.canvas.create_rectangle(position_x*5*self.cell_size, 0, (position_x+1)*self.cell_size*5, 0 + self.cell_size*6)
                self.canvas.create_text(position_x*5*self.cell_size + 30, -5, text=nom, font=("Arial", 8))
                self.canvas.create_text(position_x*5*self.cell_size + 35, 5, text=f"position_x : {position_x}")
            else:
                voisine = row["voisine"]
                if voisine:
                    voisine_row = df[df["nom"] == voisine]
                    if not voisine_row.empty:
                        representation_voisine = ast.literal_eval(voisine_row.iloc[0]["representation"])
                        y_offset = len(representation_voisine)
                        
                        for i, line in enumerate(representation):
                            for j, element in enumerate(line):
                                if element != 0:
                                    x = position_x * 5 * self.cell_size + j * self.cell_size
                                    y = (i + y_offset) * self.cell_size
                                    self.draw_shape(element, x, y, self.cell_size)
                        self.canvas.create_rectangle(position_x * 5 * self.cell_size, y_offset * self.cell_size, (position_x + 1) * self.cell_size * 5, (y_offset + len(representation)) * self.cell_size)
                        self.canvas.create_text(position_x * 5 * self.cell_size + 30, y_offset * self.cell_size - 5, text=nom, font=("Arial", 8))
                        self.canvas.create_text(position_x * 5 * self.cell_size + 35, y_offset * self.cell_size + 5, text=f"position_x : {position_x}")
        
        self.canvas.config(scrollregion=self.canvas.bbox("all"))

    #Recharge les données du fichier excel
    def refresh_cellule_comboboxes(self):
        excel_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        book = load_workbook(excel_path)
        sheet_name = "cellule"
        ...
        if sheet_name in book.sheetnames:
            ...
            self.nom_combobox['values'] = noms_existants
            self.voisine_combobox['values'] = voisines_existantes
    ...
    def draw_shape(self, shape, x, y, size):
        if shape == 'disjoncteur':
           ...
           self.dessin.dessiner_croisement(x, y , size)            
            ...
            self.dessin.dessiner_source(x, y, size)            
        elif shape == 'fusible':
            self.dessin.dessiner_fusible(x + size/2, y, size)
    ...
    def add_row(self):
        ...
        self.update_representation_cellule()  #met à jour la représentation de la cellule
    ...
    def load_cellule(self, event=None):
        ...
        if row_index:
            self.current_row_index = row_index[0]
    ...
    def create_cellule(self):
        ...
        self.canvas.delete("all")
        self.create_matrix()
        self.canvas.create_rectangle(0, 0, 0 + self.cell_size*5, 0 + self.cell_size*6)
        self.delete_cellule_button.config(state=tk.DISABLED)
    ...
    def update_matrix_ui(self, rows, cell_size, dessin, offset_x=0):
        options = [0, 0, "disjoncteur", "contacteur", "tiroir", "fusible", "smalt", "source", "fil_h", "fil_v", "transformateur",
                "coffret", "angle_haut_gauche", "angle_haut_droit", "angle_bas_gauche", "angle_bas_droit", "fil_dep_smalt",
                "fil_h_dep", "croisement"]
    ...
    def delete_cellule(self):
        ...
        # Réinitialiser la matrice et l'affichage
        self.matrix = []
        self.position_x = None
        self.canvas.delete("all")
        self.update_matrix_ui(0, self.cell_size, self.dessin)

        # Réinitialiser les boutons et les champs associés
        self.delete_cellule_button.config(state=tk.DISABLED)
        self.add_row_button.config(state=tk.DISABLED)
        self.remove_row_button.config(state=tk.DISABLED)
        ...
        # Réinitialiser le canvas matrix
        for widget in self.inner_frame.winfo_children():
            widget.destroy()
        self.canvas_matrix.config(scrollregion=self.canvas_matrix.bbox("all"))

        self.load_and_draw_all_cells()
        self.refresh_cellule_comboboxes()
    ...
    def save_cellules(self):
        ...
        for label in self.cellule_labels:
            ...
            if not value and label == 'voisine':
                new_cellule[label] = 'non'
            elif value.isdigit():
        ...
        if row_index:
            ...
            print(f"modification cellule {nom_to_save} dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
        else:
            #Ajoute une nouvelle ligne
            df = df.append(new_cellule, ignore_index=True)
            print(f"Nouvelles cellules sauvegardées dans {self.palier}_modifs_LHC.xlsx, feuille 'cellule'")
    ...
    # Réinitialiser les champs de l'onglet cellule
    def reset_cellule(self):
        print(f"Index de la cellule en cours : {self.current_row_index}")
        for label, widget in self.cellule_entries.items():
            if isinstance(widget, ttk.Combobox) or isinstance(widget, ttk.Entry):
                widget.delete(0, tk.END)

        # Réinitialiser les listes et labels
        self.voisine_list = []
        self.voisine_label.config(text="")
        ...
        self.current_row_index = None
        self.load_and_draw_all_cells()
        print(f"Onglet cellule réinitialisé. {self.current_row_index}")
    ...
    def tableauLHC_tab(self):
        etapes = EtapesChoisies()
        initialisation = InitialisationCellules(f"{self.palier}_modifs_LHC")
        tableau = ExecutionLHC(self.window)
        tableau.tableauLHC(initialisation, f"{self.palier}_modifs_LHC", etapes)
    ...
    def update_palier(self):
        modifs_path = f"dataLHC/{self.palier}_modifs_LHC.xlsx"
        palier_path = f"dataLHC/{self.palier}_LHC.xlsx"
