...
class SMALT(ElementElectrique):
    ...
    def __init__(self, nom,type, clefs, etat, cellule, representation, deplacement, appartenance):
        super().__init__(nom, clefs, cellule, type) #caracts communes a tout element electrique
    ...
    def ouverture_fermeture_SMALT(self, disjoncteur,  cellule):
        
        #la première condition pour pouvoir continuer est de ne pas avoir de contraintes mecaniques
        if disjoncteur == [] or not disjoncteur.contraintes_mecaniques_SMALT():
            M = self.representation  #représentation de la serrurerie du smalt
