...
class ElementElectrique:
    ...
    #cette liste stocke les clefs libres à chaque étapes : les clefs libres sont initialisées dans le tableau Excel    
    clefs_libres = []
    
    #chaque élément électrique devra avoir les caractéristiques suivantes
    ...
    def decaler_matrice(self): #sera utilisée seulement par les élément ayant une serrurerie (partie mobile, éclisse, smalt)
    ...
    def changer_etat_clef(self, clef_nom, nouvel_etat):
        ...
        #Vérifie si la clef a été trouvée et change son état
    ...
    #renvoie deux listes : une contenant les (objets) clefs prisonnières et l'autre le nom des clefs prisonnières
    ...
    #retourne la liste des clefs dont le pêne est sorti
    def clef_sorties(self):
        return [clef for clef in self.clefs if clef.est_sorti]
    
    #retourne la liste des clefs donc la pêne est rentré
    def clef_rentrees(self):
        return [clef for clef in self.clefs if clef.est_rentre]
